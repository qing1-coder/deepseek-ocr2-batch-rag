...
try:
    ...
    # Optional acceleration dependency; SDPA fallback is used when unavailable.
...
class NoTPAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if self.use_flash_attention and flash_attn_qkvpacked_func is not None:
            output = flash_attn_qkvpacked_func(xqkv)
            output = output.view(bsz, seqlen, -1)
            # xq, xk, xv = torch.split(xqkv, 1, dim=2)
            # xq = xq.squeeze(2)
...
class VitModel(nn.Module):
    def __init__(
        ...
        if cfg.get("fp32norm", False):
            # `logger` is not imported here; the original code path was a no-op
            # outside Megatron, so we just construct the fp32 LayerNorm directly.
