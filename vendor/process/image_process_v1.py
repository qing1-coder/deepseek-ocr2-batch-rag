...
from vendor.config import BASE_SIZE, CROP_MODE, IMAGE_SIZE, MAX_CROPS, MIN_CROPS, MODEL_PATH, PROMPT
...
class DeepseekOCRProcessor(ProcessorMixin):
    """Image processor for the original DeepSeek-OCR (model1).

    The token layout differs from DeepSeek-OCR2 and includes a per-row
    `image_newline` placeholder, mirroring the behavior of the official
    ``DeepseekOCRProcessor`` in ``DeepSeek-OCR-vllm/process/image_process.py``.
    """
    ...
    def tokenize_with_images(
        ...
        """Tokenize text with <image> tags using the model1 token layout."""
        ...
        for text_sep, image in zip(text_splits, images):
            ...
            # model1 default cropping threshold is 640 (the lowest tile size).
            ...
            # model1 inserts an extra image_newline token at the end of every row.
...
# Best-effort registration. The vllm flow instantiates the processor class
# directly, so this is only a defensive hook for AutoProcessor.from_pretrained.
try:
    AutoProcessor.register("DeepseekVLV2Processor", DeepseekOCRProcessor, exist_ok=True)
except TypeError:
    # Older transformers versions don't accept ``exist_ok``.
    try:
